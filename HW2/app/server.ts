...
// GET calls
app.get("/api/authorbyid", async (req, res: AuthorResponse) => {
    if (!req.query.id) {
...
app.get("/api/bookbyid", async (req, res: BookResponse) => {
    if (!req.query.id) {
...
app.post("/api/addauthor", async (req, res)=> {
    await db.run(
        "INSERT INTO authors(id, name, bio) VALUES('"+ req.body.id + "', '" + req.body.name + "', '" + req.body.bio + "')"
    ).catch((error) => 
    {
...
app.post("/api/addbook", async (req, res)=> {
    await db.run(
        "INSERT INTO books(id, author_id, title, pub_year, genre) VALUES('"+ req.body.id + "', '" + req.body.author_id + "', '" + req.body.title + "','" + req.body.pub_year + "','" + req.body.genre + "')"
    ).catch((error) => 
    {
...
app.delete("/api/deleteauthor", async (req, res) => {
    await db.run(
        "Delete FROM authors WHERE id = '" + req.body.id + "')"
    ).catch((error) => 
    {
